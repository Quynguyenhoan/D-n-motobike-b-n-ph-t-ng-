...
namespace Project_62131673.Areas.Admin.Controllers
{
    public class News_62131673Controller : Controller
    {
        ...
        public ActionResult Index(string Searchtext, int? page)
        {
            ...
            IEnumerable<News_62131673> items = db.News.OrderByDescending(x => x.Id);
            if (!string.IsNullOrEmpty(Searchtext))
            {
                items = items.Where(x => x.Alias.Contains(Searchtext) || x.Title.Contains(Searchtext));
            }
            var pageIndex = page.HasValue ? Convert.ToInt32(page) : 1;
            items = items.ToPagedList(pageIndex, pageSize); ViewBag.PageSize = pageSize;
            ViewBag.Page = page;
            return View(items);
        }
        ...
        public ActionResult Add(News_62131673 model)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(News_62131673 model)
        {
            if (ModelState.IsValid)
            {
                model.ModifiedDate = DateTime.Now;
